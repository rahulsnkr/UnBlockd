...
class App extends Component {
  ...
  convertToBuffer = async (reader, isFace) => {
    ...
    var aesCtr = new aesjs.ModeOfOperation.ctr(this.state.aesKey);
    if(isFace){
      var faceByteArray = new Uint8Array(reader.result);
      var faceEncrypted = aesCtr.encrypt(faceByteArray);
      const faceBuffer = await Buffer.from(faceEncrypted)
      //set this buffer -using es6 syntax
      this.setState({ faceBuffer });
    } else {
      var sigByteArray = new Uint8Array(reader.result);
      var sigEncrypted = aesCtr.encrypt(sigByteArray);
  ...
  onSubmit = async (event) => {
    ...
    await ipfs.add(this.state.faceBuffer, async (err, ipfsFaceHash) => {
      ...
      await ipfs.add(this.state.sigBuffer, (err, ipfsSigHash) => {
        ...
        storehash.methods.sendHashes(this.state.SSN, this.state.ipfsFaceHash, this.state.ipfsSigHash).send({
          from: accounts[0]
  ...
  onSSNSubmit = async (event) => {
    ...
    storehash.methods.getHashes(this.state.SSN).call({
      ...
      ipfs.files.get(res.faceHash, (err, files) => {
        var aesCtr = new aesjs.ModeOfOperation.ctr(this.state.aesKey);
        var faceDecrypted = aesCtr.decrypt(files[0].content);
        console.log(files[0].content);
        var faceBlob = new Blob( [ faceDecrypted ], { type: "image/jpeg" } );
        var faceURL = URL.createObjectURL(faceBlob);
        ...
        ipfs.files.get(res.sigHash, (err, sigFiles) => {
          ...
          var sigDecrypted = aesCtr.decrypt(sigFiles[0].content);
          var sigBlob = new Blob( [ sigDecrypted ], { type: "image/jpeg" } );
          var sigURL = URL.createObjectURL(sigBlob);
  ...
  onSSNClick = async () => {
    try {
      ...
      await web3.eth.getTransactionReceipt(this.state.SSNtransactionHash, (err, SSNtxReceipt) => {
        console.log(err, SSNtxReceipt);
        this.setState({ SSNtxReceipt });
      }); 
      await this.setState({ SSNblockNumber: this.state.SSNtxReceipt.blockNumber });
      await this.setState({ SSNgasUsed: this.state.SSNtxReceipt.gasUsed });
    } 
    catch (error) {
      console.log(error);
    } 
    ...
    return (
      <div className="App">
        ...
        >
          ...
          <Container>
            <h3> Enter SSN to retrieve face and signature images from IPFS </h3>
            ...
            <hr />
            <Button onClick={this.onSSNClick}> Get Transaction Receipt </Button>
            <Table bordered>
              <thead>
                <tr>
                  <th>Tx Receipt Category</th>
          ...
          <Tab eventKey="access" title="Access Control">
            ...
            <ImageUpload setFace={this.setFaceCallback} setSign={this.setSignCallback} mode={0}/>
            <hr />
            <h5>Select second set of images</h5>
            <ImageUpload setFace={this.setFaceCallback} setSign={this.setSignCallback} mode={1}/>
